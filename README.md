# Remove the Complexity of Building Agents
 - https://www.youtube.com/@Accelerated-AI
 - https://www.linkedin.com/in/accelerated-ai
 - https://medium.com/@accelerated-ai

--- 
### 📙+🤖 --> Agent in Colab, No Framework 
In Google Colab, build a simple agent withOUT a framework like LangChain
 - 🔟 lines 🐍
 - 📺 https://www.youtube.com/watch?v=WTb4OTTp5Ek
<br/>  
<br/>

### 🦜🔗+📙 --> LangChain Agent in Notebook 
In Google Colab, build a simple agent with classic LangChain API
 - 🔟 lines 🐍
 - 📺 https://www.youtube.com/watch?v=cZg32sVYpjg
<br/>  
<br/>

### 🦜🔗+📊 --> Plotly Agent : Prompt to Plot
In a notebook, build a simple agent that creates Plotly plots from a pandas DataFrame
- 📙 [001_plotly_langchain.ipynb](https://github.com/will-hill/Data-Science-Agents-Simplified/blob/master/001_plotly_langchain.ipynb)
- 🎥 https://www.youtube.com/shorts/LBAX1zK2W18
- 📰 https://accelerated-ai.medium.com/prompt-to-plotly-simple-notebook-agent-47bc899e0067
<br/>  
<br/>

### 🦜🔗 --> LangGraph Studio - Getting Started
Getting started with LangGraph Studio to visualize your agents
- 🐍 [002_agent.py](https://github.com/will-hill/Data-Science-Agents-Simplified/blob/master/002_agent.py)
- 📙 [002_agent.ipynb](https://github.com/will-hill/Data-Science-Agents-Simplified/blob/master/002_agent.ipynb)
- 🎥 https://www.youtube.com/shorts/36PkabnDq9A
- 📺 https://www.youtube.com/watch?v=dPdBXK0xZQI
- 📰 https://accelerated-ai.medium.com/4-steps-langgraph-studio-041a8a9a2188
<br/>  
<br/>
  

### 🦜🔗 Multi-Agents Simplified
Getting started with Multi-Agent with a very simple example
 - 🐍 [003_multi_agent.py](https://github.com/will-hill/Data-Science-Agents-Simplified/blob/master/003_multi_agent.py)
 - 📙 [003_multi_agent.ipynb](https://github.com/will-hill/Data-Science-Agents-Simplified/blob/master/003_multi_agent.ipynb)
 - 🎥 https://www.youtube.com/shorts/KkAYIujV5iY
 - 📺 https://www.youtube.com/watch?v=-om7ZpgG2SU
 - 📰 https://accelerated-ai.medium.com/multi-agent-langgraph-2689d7ea71bb
<br/>  
<br/>



### 🦜🔗 Multi-Agent Orchestration OFF v ON
Understanding and Isolating Multi-Agent Orchestration in LangGraph
 - 🐍 [004_orchestration_OFF.py](https://github.com/will-hill/Data-Science-Agents-Simplified/blob/master/004_orchestration_OFF.py)
 - 🐍 [004_orchestration_ON.py](https://github.com/will-hill/Data-Science-Agents-Simplified/blob/master/004_orchestration_ON.py)
 - 📙 [004_orch_OFF_v_ON.ipynb](https://github.com/will-hill/Data-Science-Agents-Simplified/blob/master/004_orch_OFF_v_ON.ipynb)
 - 🎥 https://www.youtube.com/shorts/PRffoVllZ7Q
 - 📺 https://www.youtube.com/watch?v=zAjKqKHY1LE
 - 📰 https://accelerated-ai.medium.com/multi-agent-orchestration-496ff7aa012b
<br/>  
<br/>


### 🤖💬+🦜🕸️ LLMs w/ SGLang & LangGraph
Local LLM Inference with SGLang and LangChain LangGraph
 - 📙 [005_SGLang.ipynb](https://github.com/will-hill/Data-Science-Agents-Simplified/blob/master/005_SGLang.ipynb)
 - 🎥 https://www.youtube.com/shorts/46Is-rMxXR8
 - 📺 https://www.youtube.com/watch?v=KaWbfpJvxmA
 - 📰 https://accelerated-ai.medium.com/sglang-for-code-gen-w-langgraph-5fa3f7dd4f25
<br/>  
<br/>


### 🛠️⚙️ + 🦜🕸️ Model Context Protocol (MCP) & LangGraph
MCP Server, Tools, Client with LangChain LangGraph
 - 📙 [006_MCP_LangGraph.ipynb](https://github.com/will-hill/Data-Science-Agents-Simplified/blob/master/006_MCP_LangGraph.ipynb)
 - 📙 [006_MCP_LangGraph_Stocks.ipynb](https://github.com/will-hill/Data-Science-Agents-Simplified/blob/master/006_MCP_LangGraph_Stocks.ipynb)
 - 🎥 https://youtube.com/shorts/jUahgNRlIMQ
 - 📺 https://www.youtube.com/watch?v=oSXNhrerlFE
 - 📰 https://medium.com/@accelerated-ai/simple-practical-mcp-langgraph-8950b42cc1cc


--- 
Backlog Topics:
- [X] Frameless Agent
- [X] Plotly Agent
- [X] LangGraph Studio
- [X] Multi-Agents
- [X] Agent Orchestration
- [X] Local LLM SGLang
- [X] MCP Tooling
- [ ] No-Code Research Agent
- [ ] LLM Inference Optimizing
- [ ] Tavily for Agents
- [ ] LLM Customized Benchmarking
- [ ] Async LLM inference
- [ ] RAG Basics
- [ ] VectorDB Basics
- [ ] FineTuning Basics
- [ ] Portfolio Agent Basics
- [ ] Forecasting Agent
- [ ] Football Modeling Agent
- [ ] Pitching Modeling Agent
- [ ] Kaggle Modeling Agent
- [ ] Robot Arm









